...
var (
  ...
  writeDelay = 5000 * time.Millisecond
...
func configHandler(w http.ResponseWriter, r *http.Request) {
  if v := r.URL.Query().Get("N"); v != "" {
    if i, err := strconv.Atoi(v); err == nil { N = i }
  }
  if v := r.URL.Query().Get("W"); v != "" {
    if i, err := strconv.Atoi(v); err == nil { W = i }
  }
  if v := r.URL.Query().Get("R"); v != "" {
    if i, err := strconv.Atoi(v); err == nil { R = i }
  }
...
func getHandler(w http.ResponseWriter, r *http.Request) {
  ...
  type result struct{ e Entry; ok bool }
  ...
  for r := range resCh {
    ...
    got++
    if r.e.Timestamp > best.Timestamp {
      best = r.e
    }
    if got >= R {
      break
